// Package cli builds the rc command tree on top of the rosettacode library.
package cli
...
type App struct {
	client *rosettacode.Client
	cfg    rosettacode.Config
...
func Root() *cobra.Command {
	app := &App{cfg: rosettacode.DefaultConfig()}

	root := &cobra.Command{
		Use:   "rc",
		Short: "Browse Rosetta Code programming tasks",
		Long: `rc reads Rosetta Code through the MediaWiki API.
No API key required. It returns records as table, JSON, JSONL, CSV, TSV, or URLs.

rc is an independent tool and is not affiliated with Rosetta Code.`,
		SilenceUsage:  true,
		SilenceErrors: true,
...
func (a *App) setup() error {
	...
	a.client = rosettacode.NewClient(a.cfg)
	return nil
}
