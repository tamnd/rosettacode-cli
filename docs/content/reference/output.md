...
describes what users get. Pick a format with `-o`, or let rc choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
rc <command> -o table   # aligned columns for reading
rc <command> -o jsonl   # one JSON object per line, for piping
rc <command> -o json    # a single JSON array
rc <command> -o csv     # spreadsheet friendly
rc <command> -o tsv     # tab-separated
rc <command> -o url     # just the URL column
rc <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
rc <command> --fields id,title,url
```
...
```bash
rc <command>            # a table, because this is a terminal
rc <command> | wc -l    # JSONL, because this is a pipe
```
